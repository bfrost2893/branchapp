<?php
  ...
  $myEmail = 'management@branchesapp.com';
  ...
  // ERROR SYNTAX FOR INVALID ZIP CODE
  // if(!isset($_POST['newsletter-zipcode']) 
  //    || !trim($_POST['newsletter-zipcode'])
  //    || (strlen($_POST['newsletter-zipcode']) != 5)
  //    || !is_numeric(trim($_POST['newsletter-zipcode']))) {
  //   response('err', 'zipcode')
  // }

  // DUPLICATING ENTRY
  $name = trim(ucfirst($_POST['newsletter-name']));
  $emailAddress = trim(strtolower($_POST['newsletter-email']));
  $zipcode = trim($_POST['newsletter-zipcode']);
  ...
  if (fwrite($file, $emailAddress . ' - ' . $name . ' - ' . $zipcode . PHP_EOL) 
          == strlen($emailAddress . ' - ' . $name . ' - ' . $zipcode . PHP_EOL)) {
    ...
    if (preg_match('/^[^@]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$/', trim($myEmail))) {
        ...
        $message = 'The following person was kind enough to test Branch:' . PHP_EOL . $name . ' - ' . $emailAddress;
        @mail($myEmail, 'You have a new beta tester', $message, $headers);
    }
